...
void RobotWheels::drivePID(int speed, int kp,
    ...
    // i+=ki*distance;
    ctrl = (int)((float)(proportional + derivative) * ((float)speed/(float)defaultSpeed));
    _mDist++;
...
{
    int dist = 0;
    leftOnTape = leftIRSensor.readThreshold(thresholdL); // adc1 ch6 = pin 34
    rightOnTape = rightIRSensor.readThreshold(thresholdR); // adc1 ch7 = pin 35
